    ...
    public class InventoryTransferer
    {
        ...
        private IEnumerator TransferingAll(ItemInventory from, ItemInventory to)
        {
            while (HasConnection)
            {
                ...
                yield return collectable.transform.DOMove(to.transform.position, 0.01f).Play().WaitForCompletion();

                if (from.ItemType == ItemType.Coin)
                {
                    GlobalEvents.SendOnAddCoin();
                }
                else if (from.ItemType == ItemType.Corn)
                {
